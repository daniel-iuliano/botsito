...
import { Play, Square, Settings, Activity, History, Zap, Globe, WifiOff } from 'lucide-react';
import { BotStatus, TradingMode, TradingConfig, BotState, ConnectionStatus, LogEntry } from './types/bot';
...
export default function App() {
  ...
  const refreshBalances = useCallback(async () => {
    try {
      const res = await apiClient.request('/api/balances');
      if (res.success) {
  ...
  useEffect(() => {
    const checkConn = async () => {
      try {
        const res = await apiClient.request('/api/auth');
        if (res.connected) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const snapshot = await apiClient.request('/api/market');
        setState(prev => ({
          ...prev,
  ...
  const toggleBot = async () => {
    if (state.status === BotStatus.OFF) {
      ...
      try {
        await apiClient.request('/api/bot', { 
          method: 'POST', 
      ...
      try {
        await apiClient.request('/api/bot', { method: 'POST', body: JSON.stringify({ action: 'STOP' }) });
      ...
      <aside className="w-16 lg:w-64 border-r border-[#1e2329] flex flex-col bg-[#0b0e11]">
        <div className="p-6 flex items-center gap-3 border-b border-[#1e2329]">
          ...
          <div className="hidden lg:block">
            ...
            <span className="text-[10px] text-gray-500 font-black uppercase tracking-tight">Next.js Stateless</span>
          </div>
        </div>
